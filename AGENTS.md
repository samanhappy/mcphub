...
- For authenticated `/api` routes, CodeQL's `Missing rate limiting` query expects explicit limiter wiring close to the protected handlers. Mount sensitive routes under `authenticatedRouteRateLimiter` (for example via a dedicated authenticated sub-router in `src/routes/index.ts`) instead of relying only on the top-level auth middleware in `src/middlewares/index.ts`.
