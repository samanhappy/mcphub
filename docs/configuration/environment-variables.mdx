...
Provider client credentials can be supplied by environment variables or by `systemConfig.auth.betterAuth.providers.oidc` from the MCPHub settings UI.
...
| `BETTER_AUTH_ALLOW_LOCAL_USER` | Allow local username/password login on `/login`. Defaults to `true`. |
| `BETTER_AUTH_AUTO_LOGIN` | Automatically redirect `/login` to OIDC when OIDC is enabled. Defaults to `true`. |
