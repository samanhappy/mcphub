...
| `JWT_SECRET` | random per-process | Secret used to sign JWTs issued by `/api/auth/login`. **Set this in production** — otherwise a new random secret is generated on every restart and existing tokens stop working. |
