...
| `OIDC_DISCOVERY_URL` | Optional helper variable for `${OIDC_DISCOVERY_URL}` interpolation inside `mcp_settings.json`. Points at your local issuer's `/.well-known/openid-configuration` endpoint. |
| `OIDC_CLIENT_ID` / `OIDC_CLIENT_SECRET` | Client credentials for a local OIDC provider such as Keycloak, Authentik, or Dex. |

For local OIDC login, configure the non-secret provider metadata in `systemConfig.auth.betterAuth.providers.oidc` and keep the client credentials in env vars. A minimal example:
