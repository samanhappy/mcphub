...
| `MCPHUB_CREDENTIAL_ENCRYPTION_KEY` | `JWT_SECRET` | Optional dedicated key material for AES-256-GCM encrypted per-user credential bindings. Use a persistent, high-entropy value. Rotation makes existing bindings unreadable, so users must re-enter them. |
...
If you enable per-user server credentials, prefer a dedicated
`MCPHUB_CREDENTIAL_ENCRYPTION_KEY`. When it is omitted, MCPHub derives the
encryption key from `JWT_SECRET`; therefore an ephemeral or rotated
`JWT_SECRET` also makes stored credential bindings unreadable. Neither key is
stored in `mcp_settings.json` or the database.
