...
`stdio` servers are launched through `npx` or `uvx`, which download and install the
server's package on first use. The image runs as `root`, so those installs land in:

| Path | Holds |
| --- | --- |
| `/root/.npm/_npx` | packages installed by `npx` |
| `/root/.npm/_cacache` | npm's downloaded tarballs |
| `/root/.cache/uv` | environments created by `uvx` |
...
Named volumes survive `docker compose down`; only `down -v` removes them. Bind mounts
(`./data/npm-cache:/root/.npm`) work equally well and are easier to inspect or clear by
hand.

A few things worth knowing:

- **Version pinning is not required.** An unpinned `npx -y <package>` still resolves
  against a warm cache — the installed package is reused without contacting the registry.
...
- **To recover from a corrupted install**, use a server's *Reinstall* action rather than
  deleting the whole cache, so only that package is refetched.

This matters most at scale. On a deployment with ~95 `stdio` servers, a cold recreate left
most of them failing their connect with `MCP error -32001: Request timed out`, because
every server was competing to install at once. With the cache mounted, the same restart
had no timeouts at all.
