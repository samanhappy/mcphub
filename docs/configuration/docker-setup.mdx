...
services:
  mcphub:
    ...
    volumes:
      ...
      - ./data:/app/data
      # Keep installed stdio server packages across container recreates.
      # See "Persisting the stdio package cache" below.
...
| Path | Holds |
| --- | --- |
| `/root/.npm/_npx` | packages installed by `npx` |
| `/root/.npm/_cacache` | npm's downloaded tarballs |
| `/root/.cache/uv` | uv's download cache and the environments `uvx` creates |

Environments created by `uv tool install` live in `~/.local/share/uv/tools` instead. Those
...
<Warning>
  `docker restart` keeps the writable layer, so it does not lose the cache.
  `docker compose down` followed by `up -d` removes and recreates the container, which
  does. If restarting appears to reinstall everything, this distinction is usually why.
</Warning>
...
Named volumes survive `docker compose down`; only `down -v` removes them. A bind mount
...
- **Version pinning is not required.** An unpinned `npx -y <package>` reuses the package
  already installed in the cache instead of reinstalling it. npm still revalidates the
  package metadata against the registry on each run, so the registry (or your proxy) must
  stay reachable — the cache removes the install cost, not the network round trip.
- **The first start after adding the volumes is still cold.** The volume begins empty, so
  that run pays the full install cost once; every recreate afterwards reuses it.
- **Budget for the size.** Around 90 `stdio` servers occupy roughly 2 GB.
- **To recover from a corrupted install**, use a server's *Reinstall* action — available
  for `npx` and `uvx` servers only. For `uvx` it refreshes just that package, through a
  `--refresh` flag on the next spawn. For `npx` it deletes the whole shared
  `/root/.npm/_npx` directory, so every npx server reinstalls on its next start, not only
  the one you targeted. The `_cacache` volume is what keeps that reinstall cheap.

This matters most at scale. On a deployment with ~95 `stdio` servers, a cold recreate left
19 of them connected and 74 failing their connect with `MCP error -32001: Request timed
out`, because every server was competing to install at once. The same restart with the
