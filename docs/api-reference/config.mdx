...
- `skipAuth` (boolean): If true, skips dashboard/web API login requirements so the frontend can call `/api` routes without a session token. This does **not** disable MCP endpoint authentication.
