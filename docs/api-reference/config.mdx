...
- `skipAuth` (boolean): If true, skips all authentication (including MCP endpoints), making the instance public. Default is `false` to protect MCP endpoints; enable only for trusted environments.
