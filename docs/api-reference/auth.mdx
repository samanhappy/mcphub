...
**Endpoints**:
The Better Auth handler is mounted at `${BASE_PATH}${betterAuthConfig.basePath}`. The default `betterAuthConfig.basePath` is `/api/auth/better`, but it is configurable through `systemConfig.auth.betterAuth.basePath` in `mcp_settings.json`. If you also set `BASE_PATH`, prepend it to every example below. Key endpoints include:
