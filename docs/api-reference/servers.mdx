...
Create several servers in a single request. The body is identical to a single `POST /api/servers` payload, but `config` is a record of `{ [serverName]: ServerConfig }`:
...
- **Body**:
  ```json
  {
    "servers": {
      ...
      "amap": { "type": "sse", "url": "https://mcp.amap.com/sse?key=..." }
    }
  }
  ```

Returns a summary of successes and failures so partial batches do not abort the whole request.
...
| **Bearer keys** | `GET /api/auth/keys`, `POST /api/auth/keys`, `PUT /api/auth/keys/:id`, `DELETE /api/auth/keys/:id` | Static tokens accepted by the dashboard / MCP transport. See [Authentication](/features/authentication). |
