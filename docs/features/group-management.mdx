...
For user-authenticated MCP requests, access is the intersection of bearer-key scope,
group visibility, and server visibility. Sharing a group never grants access to a private
server. These checks also apply to SSE and `$smart/<group>` routes and are repeated for
existing sessions. System keys retain their existing operator scope; visibility does not
turn them into user identities or provide personal credentials.
