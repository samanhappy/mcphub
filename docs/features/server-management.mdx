...
- **Copy MCP config** (group card menu): generates the configuration for the group's aggregated endpoint.
- **Copy** (server card menu): generates the configuration for that server's endpoint.
