...
<Tabs>
  ...
  <Tab title="Manual Setup">
    ...
    export SMART_ROUTING_ENABLED="true"
    ```
...
Every Smart Routing setting above takes precedence over the matching field in the dashboard: resolution order is **environment variable → dashboard setting → built-in default**. A leftover variable in `.env` therefore wins over a value you save in the UI, which is a silent failure mode rather than an error — the dashboard shows the value you typed while the runtime keeps using the env var (issue #642). Affected fields now carry a warning naming the variable that is actually in use.

The same order applies to the `Enabled` toggle and `DB URL`, but the dashboard reflects those directly instead of warning: the toggle mirrors the env var, and the URL field shows a `${DB_URL}` placeholder.
...
<AccordionGroup>
  ...
  <Accordion title="Embedding Model Selection">
    ...
    # Optional: 128, 256, 512, 768, 1536, 2048, or 3072
    EMBEDDING_DIMENSIONS=768
    ```
    ...
    <Tip>
      **Google Gemini Free Tier**
      ...
      It operates through the standard `llmProviderBaseUrl` setting — no additional
  ...
  <Accordion title="Embedding Task Prefixes">
    Asymmetric retrieval models are trained with different task prefixes for queries and
    documents, and score noticeably worse on raw text. Set the prefixes your model expects
    in Settings → Smart Routing or via environment variables:

    ```bash
    # EmbeddingGemma
    SMART_ROUTING_EMBEDDING_QUERY_PREFIX="task: search result | query: "
    SMART_ROUTING_EMBEDDING_DOCUMENT_PREFIX="title: none | text: "
    ```

    Both are empty by default, which embeds raw text as before. Prefixes are used verbatim,
    so keep the trailing space. Changing the document prefix re-embeds every server's tools;
    changing the query prefix takes effect on the next search.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Poor Search Results">
    ...
    **Solutions:**
    ...
    ```bash
    # Re-index all tools (full rebuild: clears vector_embeddings first, then
    # regenerates embeddings for every enabled server; servers that require
    # personal credentials are reached through the users holding a binding,
    # indexing the union of the tools those users can see; runs synchronously
    # and streams per-server progress through SSE embedding-sync-progress events)
  ...
  <Accordion title="Performance Issues">
    ...
    **Solutions:**
    ...
    ```bash
    # Check vector store status and performance (config, database connection,
    # row counts, per-model/per-server breakdown, sync freshness, coverage)
  ...
  <Accordion title="Authentication Errors Despite a Valid Dashboard Key">
    **Symptoms:**
    - Tool indexing fails with an authentication (401/403) error
    - A valid key is saved in the dashboard, yet requests keep being rejected

    **Cause:** an environment variable outranks the dashboard value, so the key you saved is never used (issue #642). Recent versions show a warning under the affected field naming the variable in use.

    **Solutions:**
    1. Look for `OPENAI_API_KEY` / `AZURE_OPENAI_API_KEY` in `.env*` files, your shell profile, or the container/service definition
    2. Remove the stale variable and restart, then re-index if the provider or model also changed
    3. Confirm the key actually in use with a manual request:

    ```bash
    curl -X POST "${OPENAI_API_BASE_URL:-https://api.openai.com/v1}/embeddings" \
      -H "Authorization: Bearer $OPENAI_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{"input": "test", "model": "text-embedding-3-small"}'
    ```
