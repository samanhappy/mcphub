...
<Tabs>
  ...
  <Tab title="Local Development">
    ...
    This starts both backend (port 3000) and frontend (port 5173) in development mode. The Vite dev server proxies API and MCP traffic to the backend. Local source checkout uses the default `admin` / `admin123` account in `mcp_settings.json`.
...
The default username is `admin`. For local development with `pnpm dev`, use `admin123`. Docker images do not include the local default user; if no `ADMIN_PASSWORD` environment variable is set on first launch, a random password is generated and printed to the server logs.
