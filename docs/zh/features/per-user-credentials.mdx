...
管理员或服务器所有者声明个人凭据字段，用户在**我的凭据**页面或服务器卡片入口绑定自己的值。共享用户可以查看字段名和显示名称，无法读取服务器连接配置，也无法读取其他用户的绑定。保存后的值无法回读，只能替换或删除。

## 部署配置

运行 `openssl rand -base64 32` 生成持久化密钥，在 MCPHub 环境中设置 `MCPHUB_CREDENTIAL_ENCRYPTION_KEY`。不要写入服务器定义；请单独备份并在重启时保持一致。更换或丢失密钥会使现有绑定无法解密。子进程不会继承该环境变量。
