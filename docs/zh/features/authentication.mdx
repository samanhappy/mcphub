...
1. `mcp_settings.json` 中设置 `systemConfig.auth.betterAuth.enabled = true`
2. 配置 `DB_URL`（Better Auth 当前依赖 PostgreSQL，会话不支持纯文件模式）
3. 设置 `BETTER_AUTH_URL`
4. 为对应登录方式提供客户端凭据

本地 OIDC 登录需要在 `systemConfig.auth.betterAuth.providers.oidc` 中配置：
...
`discoveryUrl` 应指向 OIDC Provider 的 `/.well-known/openid-configuration`。

如果 Dashboard 通过不同的公网来源发起登录，请设置 `systemConfig.auth.betterAuth.trustedOrigins` 来允许该来源。如果没有显式配置 `trustedOrigins`，MCPHub 会在设置了 `systemConfig.install.baseUrl` 时自动信任它的来源。
